...
const positionPanel = computed(() => {
  return '1.25rem'; // Always show panel
});
...
watch(routeName, (n) => {
  // togglePanel.value = false; // Removed to keep panel visible
  ...
  <menu
    ...
    <div class="relative bg-light-300 dark:bg-dark-800 p-6 rounded-md">
      ...
      <div class="absolute right-[105%] flex flex-col space-y-2 top-0">
        <button v-tooltip:left.tooltip="'Schema'" class="btn" @click="$router.push('/')">
          <i-ic:outline-account-tree></i-ic:outline-account-tree>
        </button>

        <button v-tooltip:left.tooltip="'AI'" class="btn" @click="$router.push('/ai')">
          <i-mdi:robot></i-mdi:robot>
          <div v-if="isAINew" class="w-4 h-4 rounded-full bg-blue-500 absolute top-3.5 -left-3 animate-ping"></div>
          <div v-if="isAINew" class="w-4 h-4 rounded-full bg-blue-500 absolute top-3.5 -left-3"></div>
        </button>

        <button v-tooltip:left.tooltip="'Settings'" class="btn !mt-12" style="display: none;">
          <i-majesticons:cog-line></i-majesticons:cog-line>
        </button>

        <button v-tooltip:left.tooltip="'Toggle Dark mode'" class="btn" @click="toggleDark()">
